...
void calculate_error2()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        weighted_sum += (float)(weights2[i]) * (line_sensor_readings.adc_reading[i]);
        sum = sum + line_sensor_readings.adc_reading[i];
    }
...
void line_follow_task(void *arg)
{
    ...
    esp_err_t err;
    err = gpio_set_direction(GPIO_NUM_15, GPIO_MODE_INPUT);
    gpio_config_t gcon;
    gcon.mode = GPIO_MODE_INPUT;
    ...
    while (true)
    {
        int ir_val = gpio_get_level(GPIO_NUM_15);
        ...
        { // IR
            ...
            while (line_sensor_readings.adc_reading[0] >= 500 || line_sensor_readings.adc_reading[1] >= 500 || line_sensor_readings.adc_reading[2] >= 500 || line_sensor_readings.adc_reading[3] >= 500)
            {
